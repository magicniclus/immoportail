...
const GetProjectProgress = () => {
    ...
    const stepName = ["Adresse", "Type de bien", "Surface", "Nombre de pièce", "Année de construction", "Travaux", "Surface", "Espaces exterieurs", "Exposition/Vue", "Standing", "Mitoyenneté", "Profil"]
    ...
    const beforeGlobalStyle = "before:w-1 before:h-3 before:absolute before:bottom-4 before:left-1.5 z-40 "
        ...
        <div className='h-full min-h-max w-6/12 my-5 flex flex-col justify-between'>
            {
                stepName.map((item, idx)=>(
                    <div className='flex'>  
                        <div className={`h-full flex justify-center mb-3 items-center relative z-0 ${ updateBefore(idx, trackStep)}`}>
                            <Icon image={trackStep > idx ? "checkRoundedPurple" : "roundedGray"} size="extraLight" /> 
                        </div>
                        <TextWithColorChange key={idx} text={item} isSelected={trackStep <= idx} valueIdx={idx} updateStyle="ml-3"/>
                    </div>
                    )
                )
            }
